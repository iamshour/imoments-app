const Privacy = () => {
	return (
		<div className='private-container'>
			...
			<p>
				If You decide to register through or otherwise grant us access using your Google's
				account, We may collect Personal data that is already associated with Your
				account, such as Your name, Your email address and your avatar.
			</p>
			<p>
				Regarding usage of Google API Services, we're obligated to accurately represent:
			</p>
			<ul>
				<li>
					<strong>Who is requesting your Google user data?</strong>The sole contributer to
					this project is imoments app developer, and the data requested so that any user
					could register is only his/her full name, email address, and avatar. This data
					is requested solely for registration purposes and ability to use the app and
					identify users between each other. No data whatsover would be exposed in any
					possible way, or shared with 3rd party services for any reason whatsover. Thus,
					any authorized client credentials to access Google API Services would be kept
					fully confidential.
				</li>
				<li>
					<strong>What data are you requesting?</strong>As mentioned before, the only data
					requested for users to register to imoments app are: Full name, email address,
					and account's avatar. User's name would be used to identify him/her among other
					users, the avatar would present his/her account's profile picture, and the email
					address is used for registering users in the database. No data would ever be
					exposed to 3rd party apps for any reason whatsover.
				</li>
				<li>
					<strong>Why are you requesting Google user data?</strong>Data requested would
					only be used to identify users among each other, as well as storing posts
					(captions or images) inside the databse, where each user's information would be
					unique and easily identifiable using eahc user's email address. Data would never
					be used for any other reason, or to any 3rd party services in any possible way
			...
			<ul>
				<li>By email: iamshour@outlook.com</li>
			</ul>
		</div>
	)
}
